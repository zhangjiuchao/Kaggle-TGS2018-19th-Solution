...
def resize_masks(masks, H, W):
    ...
    new_masks = np.zeros((batch_size, H, W), np.float32)
    for i in range(batch_size):
        m = cv2.resize(masks[i],  dsize=(W, H))
        new_masks[i] = m
...
def do_center_pad2(image, mask, pad):
    ...
    mask  = cv2.copyMakeBorder(mask,  pad, pad, pad, pad, cv2.BORDER_REPLICATE)
    return image, mask
...
def do_shift_scale_rotate2(image, mask, dx=0, dy=0, scale=1, angle=0):
    ...
    mask  = cv2.warpPerspective(mask, mat, (W, H), flags=cv2.INTER_NEAREST,
                                borderMode=border_mode, borderValue=(0, 0, 0))
    mask = (mask > 0.5).astype(np.float32)
    return image, mask
